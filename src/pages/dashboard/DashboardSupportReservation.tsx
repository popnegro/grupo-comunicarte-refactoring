...
export default function DashboardSupportReservation() {
  ...
  const save = async () => {
    ...
    try {
      const token = localStorage.getItem('admin_token');
      if (!token) return navigate('/login');
      const availableFrom = form.disponibilidad === 'reservado'
    ...
    <div className="mx-auto max-w-4xl space-y-6 pb-12">
      <header>
        ...
        <div className="flex items-start gap-3">
          <div className="rounded-xl bg-amber-50 p-2 text-amber-700"><CalendarDays className="h-5 w-5" /></div>
          <div><div className="text-eyebrow text-amber-700">PERÍODO DE RESERVA</div><h1 className="mt-1 text-page-title text-gray-900">{name || 'Soporte'}</h1><p className="mt-1 text-sm text-gray-500">Registrá el período comercial vigente. Esto no automatiza la disponibilidad.</p></div>
        </div>
      </header>
      ...
      {message && <div role="status" className="rounded-xl border border-emerald-100 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800">{message}</div>}
      ...
      <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-2xs">
        <div className="space-y-6">
          ...
          {form.disponibilidad === 'reservado' && <div className="rounded-2xl border border-amber-100 bg-amber-50/60 p-5">
          ...
          <div className="flex flex-col gap-3 border-t border-gray-100 pt-5 sm:flex-row sm:justify-end">
            <button onClick={() => navigate(`/dashboard/soportes/${encodeURIComponent(canonicalId || '')}/edit`)} className="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-xs font-bold text-gray-700">Volver a editar soporte</button>
            <button disabled={saving} onClick={save} className="inline-flex items-center justify-center gap-2 rounded-xl bg-gray-900 px-5 py-2.5 text-xs font-bold text-white disabled:opacity-50">{saving && <Loader2 className="h-4 w-4 animate-spin" />}{saving ? 'Guardando…' : 'Guardar período'}</button>
          </div>
        </div>
      </section>
    </div>
