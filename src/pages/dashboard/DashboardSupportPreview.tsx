    ...
    <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 space-y-6 pb-12">
      <header className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
          ...
          <div className="text-eyebrow text-emerald-700">VISTA PREVIA DE PUBLICACIÓN</div>
          <h1 className="mt-2 text-page-title text-gray-900">{item?.name || 'Soporte'}</h1>
          <p className="mt-1 max-w-2xl text-sm text-gray-500">Así se presenta este soporte al usuario en el inventario público.</p>
        </div>
        <div className="flex gap-2">
          <button onClick={() => navigate(`/dashboard/soportes/${encodeURIComponent(canonicalId || '')}/edit`)} className="rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-xs font-bold text-gray-700">Editar soporte</button>
          <button onClick={() => navigate(`/inventario?soporte=${encodeURIComponent(canonicalId || '')}`)} className="inline-flex items-center gap-2 rounded-xl bg-gray-900 px-4 py-2.5 text-xs font-bold text-white"><ExternalLink className="h-4 w-4" />Ver en inventario</button>
        </div>
      </header>
      ...
      {item && (
        <div className="grid gap-8 lg:grid-cols-[minmax(0,420px)_1fr] lg:items-start">
          <section>
            <div className="mb-3 text-xs font-bold uppercase tracking-[0.08em] text-gray-500">Product Card publicada</div>
          ...
          <section className="rounded-2xl border border-gray-200 bg-white p-6 shadow-2xs">
            ...
            <dl className="mt-6 space-y-4 text-sm">
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4"><dt className="font-semibold text-gray-500">Publicación</dt><dd className="font-bold text-gray-900">{item.active === false ? 'No publicado' : 'Publicado'}</dd></div>
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4"><dt className="font-semibold text-gray-500">Disponibilidad</dt><dd className="font-bold text-gray-900">{item.disponibilidad === 'reservado' ? 'Reservado' : 'Disponible'}</dd></div>
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4"><dt className="font-semibold text-gray-500">Imagen principal</dt><dd className="font-bold text-gray-900">{item.imageUrls?.[0] ? 'Configurada' : 'Sin imagen'}</dd></div>
              <div className="flex items-start justify-between gap-4 border-b border-gray-100 pb-4"><dt className="font-semibold text-gray-500">Atributos visibles</dt><dd className="font-bold text-gray-900">Hasta 2 según tipo</dd></div>
