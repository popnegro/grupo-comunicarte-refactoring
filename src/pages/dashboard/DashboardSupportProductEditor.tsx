...
type FormState = {
  ...
  coverKind: MediaKind;
  restMedia: string[];
  traditional: { formato: string; medidas: string; caras: string; impresion: string; monthly_impacts: string };
  led: { formato: string; medidas: string; resolucion: string; frecuencia: string; video_mode: string; spot_duration: string; monthly_impacts: string };
  mobile: { pantalla: string; resolucion: string; spot_duration: string; minimum_daily_outings: string; recorrido: string; operation_days: string; duration: string; monthly_impacts: string };
...
function normalize(item: any): FormState {
  ...
  return {
    ...
    traditional: { formato: text(technical.formato || technical.format), medidas: text(technical.measures), caras: text(technical.caras), impresion: text(technical.impresion), monthly_impacts: text(technical.monthly_impacts) },
    led: { formato: text(technical.formato || technical.format), medidas: text(technical.measures), resolucion: text(technical.resolution), frecuencia: text(technical.daily_frequency), video_mode: text(technical.video_mode), spot_duration: text(technical.spot_duration_seconds), monthly_impacts: text(technical.monthly_impacts) },
    mobile: { pantalla: text(technical.pantalla || technical.measures), resolucion: text(technical.resolution), spot_duration: text(technical.spot_duration_seconds), minimum_daily_outings: text(technical.minimum_daily_outings), recorrido: text(route.route_name || route.schedule || technical.summary), operation_days: text(technical.operation_days || route.weekdays), duration: text(route.duration || technical.route_duration_hours), monthly_impacts: text(technical.monthly_impacts) },
...
function payloadFrom(form: FormState) {
  ...
  let technical: any = {};
  if (form.tipo_soporte === 'tradicional') technical = { summary: form.traditional.formato, measures: form.traditional.medidas, formato: form.traditional.formato, caras: Number(form.traditional.caras || 0), impresion: form.traditional.impresion, monthly_impacts: form.traditional.monthly_impacts === '' ? null : Number(form.traditional.monthly_impacts) };
  if (form.tipo_soporte === 'led') technical = { summary: form.led.formato, measures: form.led.medidas, formato: form.led.formato, resolution: form.led.resolucion, daily_frequency: form.led.frecuencia, video_mode: form.led.video_mode, spot_duration_seconds: Number(form.led.spot_duration || 0), monthly_impacts: form.led.monthly_impacts === '' ? null : Number(form.led.monthly_impacts) };
  if (form.tipo_soporte === 'led_movil') technical = { summary: form.mobile.pantalla, measures: form.mobile.pantalla, resolution: form.mobile.resolucion, spot_duration_seconds: Number(form.mobile.spot_duration || 0), minimum_daily_outings: Number(form.mobile.minimum_daily_outings || 0), operation_days: form.mobile.operation_days, monthly_impacts: form.mobile.monthly_impacts === '' ? null : Number(form.mobile.monthly_impacts), route_duration_hours: form.mobile.duration === '' ? null : Number(form.mobile.duration) };
  ...
  return {
    ...
    ...(form.tipo_soporte === 'led_movil' ? { route: { route_name: form.mobile.recorrido, schedule: form.mobile.operation_days, duration: form.mobile.duration, route_mode: 'led_mobile', routePath: [], waypoints: [] } } : {}),
  };
}
  ...
  const previewItem = useMemo<InventoryItem>(() => {
    const family = familyFor(form.tipo_soporte); const technical: any = form.tipo_soporte === 'tradicional'
      ...
      : form.tipo_soporte === 'led'
        ...
        : { measures: form.mobile.pantalla, resolution: form.mobile.resolucion, spot_duration_seconds: Number(form.mobile.spot_duration || 0), minimum_daily_outings: Number(form.mobile.minimum_daily_outings || 0), monthly_impacts: form.mobile.monthly_impacts === '' ? null : Number(form.mobile.monthly_impacts), metadata: { cover_media_type: form.coverKind } };
    ...
    return family === 'led_mobile' ? { ...base, routePath: [], waypoints: [], schedule: form.mobile.operation_days, duration: form.mobile.duration, technical: { ...technical, monthly_impacts: form.mobile.monthly_impacts === '' ? null : Number(form.mobile.monthly_impacts) } , route: undefined } as MobileRoute : base as InventoryItem;
  }, [canonicalId, form]);
  const save = async () => {
    setError('');
    setSaving(true);
        ...
        >
          <section className={sectionClass}>
            ...
            <div className="mt-5 space-y-5">
              ...
              {form.tipo_soporte === 'tradicional' && <div className="space-y-3"><div className={labelClass}>Atributos visibles</div><div className="flex flex-wrap gap-2"><BadgeField label="Formato" value={form.traditional.formato}/><BadgeField label="Medidas" value={form.traditional.medidas}/><BadgeField label="Caras" value={form.traditional.caras}/><BadgeField label="Impresión" value={form.traditional.impresion}/></div><div className="grid gap-4 md:grid-cols-2"><Field label="Formato" value={form.traditional.formato} onChange={(v) => setNested('traditional', { formato: v })}/><Field label="Medidas" value={form.traditional.medidas} onChange={(v) => setNested('traditional', { medidas: v })}/><Field label="Impactos mensuales" value={form.traditional.monthly_impacts} onChange={(v) => setNested('traditional', { monthly_impacts: v })} type="number"/><Field label="Caras" value={form.traditional.caras} onChange={(v) => setNested('traditional', { caras: v })}/><Field label="Impresión" value={form.traditional.impresion} onChange={(v) => setNested('traditional', { impresion: v })}/></div></div>}
              {form.tipo_soporte === 'led' && <div className="space-y-3"><div className={labelClass}>Atributos visibles</div><div className="flex flex-wrap gap-2"><BadgeField label="Formato" value={form.led.formato}/><BadgeField label="Medidas" value={form.led.medidas}/><BadgeField label="Resolución" value={form.led.resolucion}/><BadgeField label="Frecuencia" value={form.led.frecuencia}/></div><div className="grid gap-4 md:grid-cols-2"><Field label="Formato" value={form.led.formato} onChange={(v) => setNested('led', { formato: v })}/><Field label="Medidas" value={form.led.medidas} onChange={(v) => setNested('led', { medidas: v })}/><Field label="Resolución" value={form.led.resolucion} onChange={(v) => setNested('led', { resolucion: v })}/><Field label="Impactos mensuales" value={form.led.monthly_impacts} onChange={(v) => setNested('led', { monthly_impacts: v })} type="number"/><Field label="Frecuencia" value={form.led.frecuencia} onChange={(v) => setNested('led', { frecuencia: v })}/><Field label="Modo de video" value={form.led.video_mode} onChange={(v) => setNested('led', { video_mode: v })}/><Field label="Duración de spot" value={form.led.spot_duration} onChange={(v) => setNested('led', { spot_duration: v })}/></div></div>}
              {form.tipo_soporte === 'led_movil' && <div className="space-y-3"><div className={labelClass}>Atributos visibles</div><div className="flex flex-wrap gap-2"><BadgeField label="Pantalla" value={form.mobile.pantalla}/><BadgeField label="Resolución" value={form.mobile.resolucion}/><BadgeField label="Spot" value={form.mobile.spot_duration}/><BadgeField label="Salidas" value={form.mobile.minimum_daily_outings}/></div><div className="grid gap-4 md:grid-cols-2"><Field label="Pantalla / formato" value={form.mobile.pantalla} onChange={(v) => setNested('mobile', { pantalla: v })}/><Field label="Duración" value={form.mobile.duration} onChange={(v) => setNested('mobile', { duration: v })}/><Field label="Impactos mensuales" value={form.mobile.monthly_impacts} onChange={(v) => setNested('mobile', { monthly_impacts: v })} type="number"/><Field label="Resolución" value={form.mobile.resolucion} onChange={(v) => setNested('mobile', { resolucion: v })}/><Field label="Duración de spot" value={form.mobile.spot_duration} onChange={(v) => setNested('mobile', { spot_duration: v })}/><Field label="Salidas mínimas" value={form.mobile.minimum_daily_outings} onChange={(v) => setNested('mobile', { minimum_daily_outings: v })}/><Field label="Recorrido" value={form.mobile.recorrido} onChange={(v) => setNested('mobile', { recorrido: v })}/><Field label="Días de operación" value={form.mobile.operation_days} onChange={(v) => setNested('mobile', { operation_days: v })}/></div></div>}
