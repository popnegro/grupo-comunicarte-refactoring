    ...
    <div className="flex flex-col w-full">
      ...
      >
        ...
        <div className="relative max-w-7xl mx-auto z-10">
          <h1 className="text-4xl md:text-5xl font-extrabold text-gray-900 tracking-tight mb-6">Nuestros Soportes</h1>
      ...
      <section className="py-24 px-4 sm:px-6 lg:px-8 bg-white">
        <div className="max-w-7xl mx-auto space-y-12">
          {soportes.map((soporte) => {
            ...
            return (
              <div key={soporte.id} className={`rounded-3xl p-8 md:p-12 flex flex-col md:flex-row gap-8 md:gap-16 items-start ${soporte.bgColor} ${soporte.textColor || 'text-gray-900'}`}>
                <div className="flex-1 space-y-6">
                  ...
                  <p className={`text-lg leading-relaxed ${isDark ? 'text-gray-300' : 'text-gray-600'}`}>{soporte.description}</p>
                  <Link to={soporte.link} className={buttonStyles({ variant: isDark ? 'default' : 'outline' })}>
                    Ver inventario disponible
                    <ArrowRight className="w-4 h-4 transition-transform group-hover:translate-x-1" />
                  </Link>
                </div>
                <div className="flex-1 w-full bg-black/5 rounded-2xl p-8 backdrop-blur-sm border border-black/5">
                  <h3 className={`text-lg font-bold mb-6 uppercase tracking-wider text-sm ${isDark ? 'text-gray-400' : 'text-gray-500'}`}>Características Principales</h3>
