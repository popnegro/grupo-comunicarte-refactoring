...
interface MultipartImage { filename: string; contentType: string; data: Buffer; }
...
function parseMultipartImage(body: Buffer, contentTypeHeader: string): MultipartImage {
  ...
  while (cursor < body.length) {
    ...
    if (disposition.filename && disposition.name && ['file', 'image'].includes(disposition.name)) {
      return { filename: disposition.filename, contentType: headerMap.get('content-type') || 'application/octet-stream', data: body.slice(dataStart, dataEnd) };
    }
    cursor = nextBoundary;
  }
...
export async function handleImageUpload(req: Request, res: Response) {
  try {
    ...
    if (!contentTypeHeader || !contentTypeHeader.toLowerCase().startsWith('multipart/form-data')) return res.status(415).json({ status: 'error', message: 'Se requiere multipart/form-data.' });
    const body = Buffer.isBuffer(req.body) ? req.body : null;
    if (!body || body.length === 0) return res.status(400).json({ status: 'error', message: 'El upload está vacío.' });
    if (body.length > MAX_IMAGE_UPLOAD_BYTES + 4096) return res.status(413).json({ status: 'error', message: 'El archivo excede el límite máximo permitido.' });
    const image = parseMultipartImage(body, contentTypeHeader);
    if (!IMAGE_MIME_TYPES.has(image.contentType.toLowerCase())) return res.status(415).json({ status: 'error', message: `MIME de imagen no permitido: ${image.contentType}.` });
    if (image.data.length === 0) return res.status(400).json({ status: 'error', message: 'El archivo está vacío.' });
    if (image.data.length > MAX_IMAGE_UPLOAD_BYTES) return res.status(413).json({ status: 'error', message: 'El archivo excede el límite máximo permitido.' });
    ...
    const uploaded = await getStorageAdapter().upload({ key, body: image.data, contentType: image.contentType.toLowerCase(), contentLength: image.data.length });
    try {
      const media = await addSupportMediaByAdmin(canonicalId, {
        media_type: 'image', url: uploaded.url, mime_type: image.contentType.toLowerCase(), sort_order: sortOrder,
        title: image.filename.replace(/\.[^.]+$/, ''), alt: image.filename.replace(/\.[^.]+$/, ''),
        metadata: { storage_key: uploaded.key, original_filename: image.filename }, active: true,
      });
