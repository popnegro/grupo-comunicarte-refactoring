...
export const getIcon = (
  ...
  const cornerBadge = isReservado
    ? `<div class="absolute -top-2 -right-2 z-[30] w-5 h-5 rounded-full bg-white border border-gray-300 flex items-center justify-center shadow-md">
         <svg xmlns="http://www.w3.org/2000/svg" width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#6b7280" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"><rect width="18" height="11" x="3" y="11" rx="2" ry="2"/><path d="M7 11V7a5 5 0 0 1 10 0v4"/></svg>
       </div>`
    : isSelected
    ? `<div class="absolute -top-2 -right-2 z-[30] w-5 h-5 rounded-full bg-emerald-500 border-2 border-white flex items-center justify-center shadow-md" aria-label="Seleccionado">
  ...
  return L.divIcon({
    ...
    html: `
      <div class="relative flex items-center justify-center w-8 h-8 transition-transform duration-200 drop-shadow-md ${activeScale} ${availabilityOpacity}">
        <div class="relative flex items-center justify-center w-8 h-8 rounded-full ${bgColor} ${activeBorder} ${activeZIndex} transition-all duration-200">
          ${svg}
        </div>
        <div class="absolute -bottom-1.5 w-3 h-3 ${bgColor} rotate-45 z-0"></div>
        ${cornerBadge}
      </div>
    `,
    iconSize: [32, 38],
