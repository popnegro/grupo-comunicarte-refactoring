...
export function MediakitPanel({ selectedItems, onClose }: MediakitPanelProps) {
  ...
  // Render at document.body level so the dialog cannot be trapped by the
  // inventory/map stacking context or painted below the site footer.
  if (typeof document === 'undefined') return null;

  return createPortal(
    <div className="fixed inset-0 z-[3000]" role="presentation">
      <div className="absolute inset-0 bg-black/40" onClick={onClose} aria-hidden="true" />
      <aside className="fixed md:absolute bottom-0 md:bottom-auto md:top-4 right-0 left-0 md:left-auto md:right-4 w-full md:w-[420px] max-h-[88vh] bg-white rounded-t-3xl md:rounded-2xl shadow-2xl overflow-hidden" role="dialog" aria-modal="true" aria-labelledby="mediakit-panel-title">
