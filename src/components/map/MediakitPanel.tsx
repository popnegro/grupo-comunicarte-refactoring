    ...
    <>
      ...
      <aside className="fixed md:absolute bottom-0 md:bottom-auto md:top-4 right-0 left-0 md:left-auto md:right-4 w-full md:w-[420px] max-h-[88vh] bg-white rounded-t-3xl md:rounded-2xl shadow-2xl z-[1500] overflow-hidden" role="dialog" aria-modal="true" aria-labelledby="mediakit-panel-title">
        <header className="flex items-center justify-between gap-4 p-5 border-b border-gray-100">
          ...
          <button type="button" onClick={onClose} className="p-2 rounded-full bg-gray-100 hover:bg-gray-200" aria-label="Cerrar Media Kit"><X className="h-5 w-5" /></button>
        </header>
        ...
        <div className="p-5 overflow-y-auto max-h-[calc(88vh-150px)]">
            ...
            {selectedItems.map((item) => (
              <div key={item.canonical_id} className="flex items-center gap-3 rounded-xl border border-gray-100 bg-gray-50 px-3 py-3">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="text-xs text-gray-500 truncate">{isMobileRoute(item) ? 'Soporte móvil' : item.tipo_soporte}</p>
                </div>
                <button type="button" onClick={() => removeSelected(item.canonical_id)} className="text-xs font-semibold text-gray-500 hover:text-gray-950 shrink-0" aria-label={`Quitar ${item.name}`}>Quitar</button>
              </div>
            ))}
          </div>
          ...
          <div className="mt-6 grid gap-2 sm:grid-cols-2">
            <Link to="/contacto?origen=mediakit" onClick={onClose} className="inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-black px-5 text-sm font-bold text-white hover:bg-gray-800">
              Solicitar Media Kit <ArrowRight className="h-4 w-4" />
            </Link>
            <button type="button" onClick={onClose} className="h-11 rounded-xl border border-gray-200 px-5 text-sm font-semibold text-gray-900 hover:bg-gray-50">Seguir seleccionando</button>
          </div>
        </div>
      </aside>
    </>
